"""ZIMT with GRPO fine-tuning + magnitude-weighted endpoint correction.

Loads the GRPO-finetuned checkpoint (trained against RF features as reward).
Uses magnitude-weighted endpoint correction (same as zimt_magcorr).
"""
...
_CKPT_PATH = (
    ...
    else Path("training/zimt_grpo_best.pt")
)

if not _CKPT_PATH.exists():
    _CKPT_PATH = _DATA_DIR / "zimt_best.pt"
    print(f"[zimt_grpo] WARNING: GRPO checkpoint not found, falling back to {_CKPT_PATH}")
...
_phase = _ckpt.get("phase", "?")
_grpo_iter = _ckpt.get("grpo_iter", "?")
_grpo_auc = _ckpt.get("grpo_auc", "?")
print(
    f"[zimt_grpo] ZIMT ({_cfg['d_model']}d, {_cfg['n_layers']}L, "
    f"{_cfg['n_components']}K MDN) | "
    f"phase={_phase}, grpo_iter={_grpo_iter}, grpo_auc={_grpo_auc} | "
