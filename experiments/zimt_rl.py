"""ZIMT RL-finetuned model with magnitude-weighted endpoint correction.

Same inference as zimt_magcorr but loads from RL-trained checkpoint.
"""
...
_CKPT_PATH = (
    ...
    else Path("training/zimt_rl_best.pt")
)

if not _CKPT_PATH.exists():
    raise FileNotFoundError(f"RL checkpoint not found: {_CKPT_PATH}")
...
print(
    f"[zimt_rl] ZIMT-RL ({_cfg['d_model']}d, {_cfg['n_layers']}L, "
...
def generate_path(
    ...
    if abs(err_x) > 0.01 or abs(err_y) > 0.01:
        ...
        total_moving_mag = sum(m for m, mv in zip(step_mags, moving_mask) if mv)
        if total_moving_mag > 0.1:
            cum_corr_x, cum_corr_y = 0.0, 0.0
            for i in range(len(step_mags)):
                if moving_mask[i]:
                    w = step_mags[i] / total_moving_mag
                    cum_corr_x += err_x * w
                    cum_corr_y += err_y * w
