"""VQ-VAE + transformer with stall token MASKED at inference.

Ablation experiment: identical to vqvae_ar_transformer.py except token 0
(stall) is set to probability 0 before sampling. If AUC gets worse, it
proves the stall token carries real signal for trajectory realism.
"""
...
print(
    f"[vqvae_ar_transformer_NO_STALL] VQ-VAE ({_vqvae_cfg['n_codes']} codes) + "
...
with torch.no_grad():
    ...
    _TOKEN_DXDY[0] = (0.0, 0.0)  # stall token (will never be sampled)
...
def generate_path(
    ...
    """Generate a trajectory with stall token masked (token 0 probability = 0)."""
    ...
    with torch.no_grad():
        for _ in range(n_target - 2):
            ...
            # ABLATION: mask stall token (token 0) — set its probability to 0
            probs[0] = 0.0
